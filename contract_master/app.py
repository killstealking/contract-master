...
if __name__ == "__main__":
    ...
    if args is not None:
        if args.platform == "bsc":
            quicknode_endpoint = (
                args.quicknode_endpoint if args.quicknode_endpoint else os.getenv("QUICKNODE_BSC_ENDPOINT")
            )
        ...
        if args.user_address is not None and args.contract_address is not None:
            ...
            bep20_balance = contract_master.get_bep20_token_balance(
                contract_address=args.contract_address, user_address=args.user_address, block_height=block_height
            )
